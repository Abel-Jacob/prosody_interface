...
ASR_DEVICE = os.getenv("ASR_DEVICE", "cuda" if _HAS_GPU else "cpu")
ASR_COMPUTE_TYPE = os.getenv("ASR_COMPUTE_TYPE", "float16" if _HAS_GPU else "int8")
WHISTRESS_DEVICE = os.getenv("WHISTRESS_DEVICE", "cuda" if _HAS_GPU else "cpu")
WHISTRESS_WHISPER_BACKBONE = os.getenv("WHISTRESS_WHISPER_BACKBONE", "openai/whisper-small.en")

# Silero VAD
VAD_THRESHOLD = float(os.getenv("VAD_THRESHOLD", "0.5"))         # Speech probability threshold
VAD_MIN_SPEECH_MS = int(os.getenv("VAD_MIN_SPEECH_MS", "250"))     # Minimum speech duration (ms)
VAD_MIN_SILENCE_MS = int(os.getenv("VAD_MIN_SILENCE_MS", "500"))    # Minimum silence between chunks (ms)
VAD_TARGET_CHUNK_SEC = float(os.getenv("VAD_TARGET_CHUNK_SEC", "25.0")) # Target ~25 seconds per chunk
VAD_MAX_CHUNK_SEC = float(os.getenv("VAD_MAX_CHUNK_SEC", "29.0"))    # Hard max just under Whisper's 30s limit
...
def _resolve_lexirep_checkpoint(key: str) -> Path:
    ...
    # 3. Local standalone server checkpoints folder (BASE_DIR / checkpoints)
    isle_dir = BASE_DIR.parent / "ISLE 768"
    local_candidates = [
        BASE_DIR / "checkpoints" / f"final_lexirep_model_{key}.pt",
        BASE_DIR / "checkpoints" / "final_lexirep_model.pt",
